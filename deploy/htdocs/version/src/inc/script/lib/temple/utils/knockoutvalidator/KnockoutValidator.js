...
define(["require", "exports", 'knockout', "lib/temple/core/Destructible"], function (require, exports, ko, Destructible_1) {
    /**
     *  ## KnockoutValidator
    ...
    var KnockoutValidator = (function (_super) {
        ...
        KnockoutValidator.prototype.validate = function () {
            ...
            if (!(this instanceof KnockoutValidator)) {
                console.warn("KnockoutValidator: validate() called from the wrong scope.");
                if (typeof this._fields == 'undefined') {
                    return;
                }
            }
            if (this._isValidating) {
                console.warn("KnockoutValidator: calling the validate() while already validating.");
                return null;
            }
        ...
        KnockoutValidator.init = function () {
            if (!this._initialized) {
                if (typeof document.body.addEventListener == "undefined") {
                    console.warn("KnockoutValidator does not support this browser!");
                }
                if (typeof document.body.classList == "undefined") {
                    console.warn("KnockoutValidator requires a browser with classList support. Please update your browser" +
            ...
            if (!field) {
                ...
                if (radioName !== null) {
                    ...
                    if (tagName == 'input' || tagName == 'textarea') {
                        ...
                        console.warn('KnockoutValidator: validation does not work on ' + tagName + ' elements.');
                        return;
                    }
            ...
            function processRuleResult(result, inverse) {
                ...
                if (PromiseUtils.isPromise(result)) {
                    ...
                    result.then(function (promiseResult) {
                        ...
                        console.warn('KnockoutValidator: one of the validation rules rejected a promise with message: ' + message);
                    });
                    return;
                }
