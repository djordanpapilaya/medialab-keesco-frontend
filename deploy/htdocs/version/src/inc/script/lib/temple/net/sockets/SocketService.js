...
define(["require", "exports", "lib/temple/events/EventDispatcher", "lib/temple/net/sockets/SocketEvent", "lib/temple/utils/MobileDetect"], function (require, exports, EventDispatcher_1, SocketEvent_1, MobileDetect_1) {
    /**
     *
     * @module Temple
    ...
    var SocketService = (function (_super) {
        ...
        SocketService.prototype.forceConnect = function () {
            ...
            if (this._servers.length > 0 && this._currentServerIndex < this._servers.length) {
                ...
                if (this.debug)
                    console.log("SocketService::Connect to server: '" + (this._protocol + this._servers[this._currentServerIndex] + ':' + this._port) + "'");
                if (this.debug)
                    console.log("SocketService::Protocols: ", this.getProtocols());
        ...
        SocketService.prototype.getServers = function () {
            var _this = this;
            if (this.debug)
                console.log("SocketService::getServers: '" + this.url + "'");
            $.ajax(this.url, {
                timeout: 5000,
                dataType: "jsonp",
                ...
                if (data && data.servers && data.servers.length > 0) {
                    if (_this.debug)
                        console.log('SocketService::Servers loaded', data);
                    ...
                    if (_this.debug)
                        console.log('SocketService::Unable to load servers');
                    _this.reconnect();
                }
            }).error(function () {
                _this.dispatchEvent(new SocketEvent_1.default(SocketEvent_1.default.NO_SERVERS_AVAILABLE));
        ...
        SocketService.prototype.reconnect = function () {
            ...
            if (this._servers.length == 0) {
                if (this.debug)
                    console.log('SocketService::No servers loaded');
                this._servers = [];
                this._currentServerIndex = 0;
                ...
                this._currentServerIndex = 0;
                if (this.debug)
                    console.log('SocketService::No servers left');
                ...
                if (this.debug)
                    console.log('SocketService::Try next server');
                this._currentServerIndex++;
                delay = 0;
            }
            if (this.debug)
                console.log('SocketService::Reconnect( ' + delay + ' )');
        ...
        SocketService.prototype.addEventListeners = function (socket) {
            ...
            if (this.debug)
                console.log('SocketService::AddEventListeners( ' + this._uniqueCallbackId + ' )');
        ...
        SocketService.prototype.handleSocketOpened = function (uniqueCallbackId) {
            if (uniqueCallbackId != this._uniqueCallbackId) {
                if (this.debug)
                    console.log('SocketService::Callback::Opened::Conflict( ' + uniqueCallbackId + ', ' + this._uniqueCallbackId + ' )');
                return;
            }
            if (this.debug) {
        ...
        SocketService.prototype.handleSocketClosed = function (uniqueCallbackId, e) {
            if (uniqueCallbackId != this._uniqueCallbackId) {
                if (this.debug)
                    console.log('SocketService::Callback::Closed::Conflict( ' + uniqueCallbackId + ', ' + this._uniqueCallbackId + ' )');
                return;
            }
            if (this.debug) {
        ...
        SocketService.prototype.handleSocketMessage = function (uniqueCallbackId, e) {
            if (uniqueCallbackId != this._uniqueCallbackId) {
                if (this.debug)
                    console.warn('SocketService::Callback::Message::Conflict( ' + uniqueCallbackId + ', ' + this._uniqueCallbackId + ' )');
                return;
            }
            if (this.debug) {
            ...
            if (this.debug)
                console.log('SocketService::', message.t + ": " + message.a, message.d);
