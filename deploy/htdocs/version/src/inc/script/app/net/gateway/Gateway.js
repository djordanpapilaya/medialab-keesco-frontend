...
define(["require", "exports", "bluebird", "lib/temple/core/Destructible", "./CachedCall"], function (require, exports, Promise, Destructible_1, CachedCall_1) {
    /**
    ...
    var Gateway = (function (_super) {
        ...
        function Gateway(options, debug) {
            ...
            if (!this._options.outputHandler) {
                console.warn('missing property outputHandler in Gateway options, falling back to default behavior');
            }
        ...
        Gateway.prototype.get = function (action, data, options) {
            ...
            if (this._runningCalls[options.cacheKey]) {
                console.error('duplicate call: ', options.cacheKey);
            ...
            if (p) {
                if (this.debug) {
                    console.info('[Gateway] result \'' + action + '\' coming from cache!');
                }
                return p;
            }
        ...
        Gateway.prototype.execute = function (action, data, options) {
            ...
            if (options === void 0) { options = {}; }
            if (this.debug)
                console.log('[Gateway] execute', action, data);
            ...
            var p = new Promise(function (resolve, reject) {
                ...
                if (_this.debug) {
                    console.log('[Gateway] canceled pending request');
                }
                // abort the request
                jqXHR.abort();
