module.exports = function(grunt)
{
	...
	grunt.registerMultiTask('scaffold-pages', 'Scaffold pages from the Gaia Sitemap', function()
	{
		...
		var branchesContent = grunt.file.read(path.join(grunt.config.data.gruntDir, 'tasks/scaffold-pages/branches.txt'));
		...
		var scaffoldPage = function (page, type, path)
		{
			...
			var defaultPageController = defaultFileName + 'PageController';
			var defaultPageViewModel = defaultFileName + 'PageViewModel';
			...
			check(createController, page, page.controller, controllerContent, defaultPageController, outputDir, outputDirMobile);
			check(createViewModel, page, page.viewModel, viewmodelContent, defaultPageViewModel, outputDir, outputDirMobile);
		...
		var define = function()
		{
			...
			if( arguments.length == 1 ){
				...
				var stubs = arguments[0].map(function() { return {}; });

				arguments[1].apply(null, stubs);

				sitemap = stubs[1].default;
			}
			...
			createPagesFile(branchesContent, pagesPath, constList);
		};
	...
	function createTemplate(content, path, page)
	{
		if(!grunt.file.exists(path))
		{
			...
			grunt.file.write(path, content
				...
				.replace(/\{page\.title\}/gim, page.title || page.id)
			);
		}
	}
	...
	function getViewModelPath(page, isMobile)
	{
		...
		{
			return path + DefaultPageViewModel;
		}
		else if(page.viewModel == 'mobile')
		{
			if(isMobile)
			{
				return pathMobile + DefaultPageViewModel;
			}
			else
			{
				return path + DefaultPageViewModel;
